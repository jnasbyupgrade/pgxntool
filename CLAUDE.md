...
### verify-results Feature

The `verify-results` feature (enabled by default) prevents accidentally running `make results` when tests are failing. It works by checking for the existence of `regression.diffs` before allowing `make results` to proceed.

**Important**: `verify-results` works with **any tests that use the pgxntool framework**, not just pgTap tests. It checks for `regression.diffs` which is generated by `pg_regress` for any test failures, regardless of whether the tests use pgTap or plain SQL. The feature is enabled by default for all pgxntool projects and can be disabled by setting `PGXNTOOL_ENABLE_VERIFY_RESULTS=no`.
